...
/**
 * Returns the active geofence configuration.
 *
 * Priority order:
 *   1. dbOverride — values saved via PUT /api/settings/geofence-db (passed in by the caller)
 *   2. .env variables — GEOFENCE_LAT, GEOFENCE_LNG, GEOFENCE_RADIUS (fallback)
 *
 * The dbOverride object is optional. When provided it should have the shape:
 *   { geofenceLatitude, geofenceLongitude, geofenceRadius }
 * (i.e. the raw fields from the OrganizationSettings document).
 */
function getGeofenceConfig(dbOverride = null) {
  // Prefer DB values when they are all valid numbers
  ...
  const latitude = hasDbGeofence ? dbLat : toNumber(process.env.GEOFENCE_LAT);
  ...
  return {
    ...
    source: hasDbGeofence ? "db" : "env",  // helpful for debugging
  };
}
...
function validateLocationPayload(payload = {}, config = getGeofenceConfig()) {
  ...
  return {
    ok: true,
    location: { latitude, longitude, accuracy, mocked, capturedAt },
  };
}
